...
// 下载整本书
app.post('/api/book/download', async (req, res) => {
  try {
    const { cookie, bookId, bookName, bookSlug } = req.body;
    ...
    // 获取文档列表
    const docsResponse = await axios.get(`https://www.yuque.com/api/docs?book_id=${bookId}`, {
      headers: {
        ...yuqueHeaders,
        Cookie: cookie
      }
    ...
    // 获取目录结构
    const catalogResponse = await axios.get(`https://www.yuque.com/api/catalog_nodes?book_id=${bookId}`, {
      headers: {
        ...yuqueHeaders,
        Cookie: cookie
      }
    ...
    // 使用DFS下载所有文档
    await dfs(tree, "0", bookFolder, cookie, nodeInfo, loginName, bookSlug);
    ...
    res.setHeader('Content-Disposition', `attachment; filename=${encodeURIComponent(bookName)}.zip`);
    ...
    console.error('Error downloading book:', error.response?.data || error.message);
    res.status(500).json({ error: error.message });
  }
});
